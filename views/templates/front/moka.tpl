...
<div class="moka-checkout">
    {if $api_environment == 'test'}
        <div class="moka-checkout-test-alert">{l s='Test Mode' mod='moka'}</div>
    {/if}
    <div class="moka-checkout-form">
        <div class="moka-checkout-input-container moka-checkout-border-radius--top-left moka-checkout-border-radius--top-right">
            ...
            <input type="text" class="moka-checkout-input" id="moka-checkout-holder" placeholder="{l s='Cardholder Name' mod='moka'}">
        </div>
        <div class="moka-checkout-input-container">
            ...
            <input type="text" class="moka-checkout-input" id="moka-checkout-card-number" placeholder="{l s='Card Number' mod='moka'}">
            <div class="moka-checkout-input-icons">
                <img src="{$link->getMediaLink("`$module_dir`views/svg/visa.svg")}" class="moka-checkout-input-icons-icon moka-checkout-visa-icon" alt="visa-logo">
                <img src="{$link->getMediaLink("`$module_dir`views/svg/mastercard.svg")}" class="moka-checkout-input-icons-icon moka-checkout-mastercard-icon" alt="mastercard-logo">
                <img src="{$link->getMediaLink("`$module_dir`views/svg/amex.svg")}" class="moka-checkout-input-icons-icon moka-checkout-amex-icon" alt="amex-logo">
                <img src="{$link->getMediaLink("`$module_dir`views/svg/troy.svg")}" class="moka-checkout-input-icons-icon moka-checkout-troy-icon" alt="troy-logo">
            </div>
        </div>
        <div class="moka-checkout-input-container moka-checkout-input-container--half moka-checkout-border-radius--bottom-left">
            ...
            <input type="text" class="moka-checkout-input" id="moka-checkout-expire-date" placeholder="{l s='Month / Year' mod='moka'}">
        </div>
        <div class="moka-checkout-input-container moka-checkout-input-container--half moka-checkout-border-radius--bottom-right">
            ...
            <input type="text" class="moka-checkout-input" id="moka-checkout-cvc" placeholder="{l s='CVC' mod='moka'}">
            <div class="moka-checkout-input-icons">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("input[name='payment-option']").on("change", function() {
                $("#payment-confirmation button").css("opacity", 1);

                if($(this).data("module-name") == 'moka') {
                    $("#payment-confirmation button").css("opacity", 0);
                }
            });
            ...
            $("#moka-checkout-pay-button").on("click", function() {
                ...
                var contract = $("input[id='conditions_to_approve[terms-and-conditions]']");

                if (contract.length > 0 && !contract.prop('checked')) {
                    return mokaErrorMessage(errorContractText);
                }

                $.ajax({
                    url: '/module/moka/checkout',
                    method: 'POST',
                    dataType: 'json',
                    data: {
                    ...
                    beforeSend: function() {
                        $("#moka-checkout-pay-button").prop('disabled', true);
                        $(".moka-checkout-message-container").remove();
                    },
                    complete: function() {
                        $("#moka-checkout-pay-button").prop('disabled', false);
                    },
                    success: function(data) {
                        ...
                        if (errors && Object.keys(errors).length > 0) {
                            ...
                            if (firstErrorMessage) {
                                mokaErrorMessage(firstErrorMessage)
                            }
                        }
            ...
            function mokaErrorMessage(errorMessage) {
                $(".moka-checkout-message-container").remove();

                var errorMessageContainer = $('<div class="moka-checkout-message-container"></div>');
                var errorLabel = $('<div class="moka-checkout-error-label">' + errorMessage + '</div>');
                $(".moka-checkout-form").append(errorMessageContainer.append(errorLabel));

                document.querySelector(".moka-checkout").scrollIntoView({ behavior: 'smooth' });
            }
        });
    </script>
{/literal}